...
const Expenses = () => {
  ...
  const handleDeleteExpense = async (
    ...
    if (
      window.confirm(
        `Are you sure you want to permanently delete Expense Voucher "${expenseId}" worth ${settings.currency} ${amount.toLocaleString()}?`
      )
    ) {
      try {
        ...
        if (
          ...
          alert(
            `Expense Voucher ${expenseId} removed successfully!`
          );
        }
      } catch (error) {
        alert(
          error.response?.data?.message ||
            'Failed to remove expense record.'
        );
      }
    }
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (
        ...
        alert(
          'Expense Voucher registered successfully!'
        );
      ...
      setModalError(
        error.response?.data?.message ||
          'Failed to save expense details.'
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  const totalFilteredExpensesVal =
  ...
  const handleDownloadCSV = () => {
    if (
      ...
      return alert(
        'No expense logs found for this selected date range.'
      );
    }

    const headers = [
      'Voucher ID,Expense Title,Category,Amount,Date & Time,Notes'
    ];
    ...
    link.setAttribute(
      'download',
      `Dukan_Expenses_Report_${filterPreset}_${new Date()
        .toISOString()
        .split('T')[0]}.csv`
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-gray-200 p-5 rounded-xl shadow-sm">
          ...
          <p className="text-2xl font-extrabold text-red-600 mt-1">
            {settings.currency}{' '}
            {totalFilteredExpensesVal.toLocaleString()}
          </p>
        </div>
      </div>

...
import { ArrowLeft, User, CreditCard, Layers, ShoppingBag, CheckCircle, Calculator, X, AlertCircle, MessageCircle } from 'lucide-react';
...
const InstallmentPlanDetails = () => {
  ...
  const handlePaySubmit = async (e) => {
    ...
    if (amt > selectedInst.remainingAmount) {
      setPayError(`Payment amount cannot be larger than outstanding dues (${selectedInst.remainingAmount}).`);
      setIsSubmitting(false);
      return;
    }

    try {
      const res = await api.post(`/installments/installment/${selectedInst._id}/pay`, {
        amount: amt,
        paymentMethod
      });
      if (res.data && res.data.success) {
        alert('Payment recorded successfully! Balance recalculated and carried forward.');
        setShowPayModal(false);
        fetchPlanDetails();
      }
    } catch (error) {
      setPayError(error.response?.data?.message || 'Failed to submit payment.');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleSendWhatsAppReminder = (inst) => {
    ...
    if (!phone) return alert('Customer mobile number is missing or invalid.');

    const dueDate = new Date(inst.dueDate);
    dueDate.setHours(23, 59, 59, 999);
    const today = new Date();
    const dueDateFormatted = new Date(inst.dueDate).toLocaleDateString('en-PK');
    const isPastDue = today > dueDate || inst.status === 'Overdue';
  ...
  if (loading && !data) {
    return (
      <div className="flex flex-col items-center justify-center p-10 space-y-3">
        ...
        <span className="text-gray-500 text-sm">Compiling monthly installments calendar...</span>
      </div>
    );
  }

  if (!data) return <div className="text-center p-10 text-red-600">Error: Plan data missing.</div>;

  const { plan, installments } = data;

  const originalPrice = plan?.sale?.finalTotal || (plan?.totalAmount - (plan?.totalAmount - plan?.downPayment - plan?.remainingBalance)) || 0;
  const remainingPrincipal = originalPrice - (plan?.downPayment || 0);
  const markupAmount = (plan?.totalAmount || 0) - originalPrice;
  const totalFinanced = (plan?.totalAmount || 0) - (plan?.downPayment || 0);
  const invoiceBillNumber = plan?.sale?.saleId || plan?.planId || 'N/A';

  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center space-x-3">
        <Link to="/installments" className="p-2 hover:bg-gray-200 rounded-lg transition-colors text-gray-600">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          {/* INVOICE / BILL NUMBER AS PRIMARY HEADER */}
          <h2 className="text-2xl font-bold text-gray-900 font-sans">
            Schedule Sheet — Bill / Invoice #: <strong className="text-indigo-600">{invoiceBillNumber}</strong>
          </h2>
          <p className="text-sm text-gray-600 font-medium">Plan #{plan?.planId} ({plan?.duration} Months) — Process payments and check real-time dynamic schedules.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex items-center space-x-4">
          ...
          <div className="truncate">
            <span className="text-[10px] uppercase font-bold text-gray-400">Buyer</span>
            <h4 className="font-bold text-gray-900 truncate">{plan?.customer?.fullName || 'N/A'}</h4>
            <p className="text-xs text-gray-500">{plan?.customer?.mobileNumber || ''}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex items-center space-x-4">
          ...
          <div className="truncate">
            <span className="text-[10px] uppercase font-bold text-gray-400">Financed Item</span>
            <h4 className="font-bold text-gray-900 truncate">{plan?.product?.name || 'Deleted Product'}</h4>
            <p className="text-xs text-gray-500">{plan?.product?.brand || ''} / {plan?.product?.model || ''}</p>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm flex items-center space-x-4">
          ...
          <div>
            <span className="text-[10px] uppercase font-bold text-gray-400">Dues Outstanding</span>
            <h4 className="font-extrabold text-red-600 text-lg">{settings.currency} {(plan?.remainingBalance || 0).toLocaleString()}</h4>
            <p className="text-xs text-gray-500">Duration: {plan?.duration || 0} Months</p>
          </div>
        </div>
      </div>

      {/* Schedulers sheet list */}
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-100 bg-gray-50/50">
          <h3 className="font-extrabold text-sm text-gray-700 uppercase tracking-wider">Plan Installments List</h3>
        </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {installments && installments.map((inst) => (
                <tr key={inst._id} className="hover:bg-gray-50/50">
                  <td className="px-6 py-4 text-gray-900 font-bold">Installment #{inst.installmentNumber}</td>
                  <td className="px-6 py-4">{settings.currency} {inst.amount.toLocaleString()}</td>
                  <td className="px-6 py-4 text-green-600 font-bold">{settings.currency} {inst.paidAmount.toLocaleString()}</td>
                  <td className="px-6 py-4 text-gray-800 font-bold">{settings.currency} {inst.remainingAmount.toLocaleString()}</td>
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold border ${
                      inst.status === 'Paid' ? 'bg-green-50 border-green-200 text-green-700' :
                      inst.status === 'Overdue' ? 'bg-red-50 border-red-200 text-red-700 font-bold animate-pulse' :
                  ...
                  <td className="px-6 py-4 text-center">
                    <div className="flex items-center justify-center space-x-1.5">
                      {inst.status !== 'Paid' ? (
                        ...
                        <span className="text-xs text-green-600 font-bold">Closed</span>
                      )}
                    </div>
                  </td>
                </tr>
      ...
      {showPayModal && selectedInst && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <form onSubmit={handlePaySubmit} className="bg-white border rounded-2xl w-full max-w-md shadow-xl overflow-hidden animate-scale-in">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-slate-50 p-4 border rounded-xl space-y-2 text-xs font-semibold text-slate-700">
                <div className="flex justify-between">
                  ...
                  <span>{settings.currency} {selectedInst.amount.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span>Remaining Months Unpaid:</span>
                  <span>{installments.filter(inst => inst.status !== 'Paid' && inst._id !== selectedInst._id).length} Months</span>
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold uppercase text-gray-500">Manual Received Amount ({settings.currency})</label>
                <input
                  type="number"
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['Cash', 'Easypaisa', 'JazzCash', 'Bank Transfer', 'Other'].map(method => (
                    <button
                      key={method}
                      type="button"
            ...
            <div className="p-4 bg-gray-50 border-t flex justify-end">
              ...
              >
                {isSubmitting ? 'Processing Payment...' : 'Record Payment & Adjust Future Dues'}
              </button>
            </div>
          </form>
        </div>

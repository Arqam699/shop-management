...
import { Plus, Search, Edit2, Trash2, FileSpreadsheet, AlertTriangle, Lock, Calendar } from 'lucide-react';
...
const Inventory = () => {
  ...
  const filteredProducts = products.filter(p => {
    ...
    const matchesSearch = name.includes(term) || brand.includes(term) || model.includes(term) || sku.includes(term);
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Product ID,Product Name,Category,Brand,Model,Purchase Price,Sale Price,Qty,Status,Added Date'];
    const rows = filteredProducts.map(p => {
      const dateFormatted = new Date(p.createdAt || p.purchaseDate).toLocaleDateString('en-PK');
    ...
    link.setAttribute("download", `Inventory_Report_${filterPreset}_${new Date().toISOString().split('T')[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  const totalStockVal = filteredProducts.reduce((acc, p) => acc + (p.purchasePrice * p.quantity), 0);
  const totalStockQty = filteredProducts.reduce((acc, p) => acc + p.quantity, 0);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 font-sans">Dukan Inventory Manager</h2>
          <p className="text-sm text-gray-600">Track stock levels, purchase/sale pricing parameters and purchase dates.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block">Filtered Products</span>
          <p className="text-xl font-extrabold text-gray-900 mt-1">{filteredProducts.length}</p>
        </div>
        <div className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block">Total Stock Qty</span>
          <p className="text-xl font-extrabold text-gray-900 mt-1">{totalStockQty}</p>
        </div>
        <div className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block">Inventory Value (Cost)</span>
          <p className="text-xl font-extrabold text-gray-900 mt-1">{settings.currency} {totalStockVal.toLocaleString()}</p>
        </div>
        <div className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider block">Low / Out of Stock</span>
            <p className="text-xl font-extrabold text-red-600 mt-1">
              {lowStockCount} <span className="text-xs text-gray-400 font-normal">Low</span> / {outOfStockCount} <span className="text-xs text-gray-400 font-normal">Out</span>
            </p>
          </div>
          {(lowStockCount > 0 || outOfStockCount > 0) && <AlertTriangle className="w-6 h-6 text-amber-500 shrink-0" />}
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-10 text-center flex flex-col items-center justify-center space-y-3">
            ...
            <span className="text-gray-500 text-sm">Querying database...</span>
          </div>
            ...
            <table className="w-full border-collapse text-left text-sm text-gray-600">
              ...
              <tbody className="divide-y divide-gray-200 font-medium">
                {filteredProducts.map((p) => (
                  <tr key={p._id} className="hover:bg-gray-50/75 transition-colors">
                    <td className="px-6 py-4 text-indigo-600 font-bold tracking-wider">{p.sku}</td>
                    <td className="px-6 py-4 font-bold text-gray-900">{p.name}</td>
                    <td className="px-6 py-4 text-gray-700">{p.brand} / {p.model}</td>
                    <td className="px-6 py-4 text-gray-500">{p.category}</td>
                    <td className="px-6 py-4 text-gray-900 font-semibold">{p.quantity}</td>
                    <td className="px-6 py-4">{settings.currency} {p.purchasePrice.toLocaleString()}</td>
                    <td className="px-6 py-4 font-semibold text-gray-800">{settings.currency} {p.salePrice.toLocaleString()}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border ${
                        p.status === 'Available' ? 'bg-green-50 border-green-200 text-green-700' :
                        p.status === 'Low Stock' ? 'bg-amber-50 border-amber-200 text-amber-700' :

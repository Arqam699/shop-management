...
if (loading) {
return ( <div className="flex flex-col items-center justify-center p-12 space-y-3"> <div className="w-10 h-10 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div> <span className="text-gray-500 text-sm font-semibold">Compiling customer profile & guarantors...</span> </div>
);
}

if (errorMsg || !customer) {
return ( <div className="bg-red-50 border border-red-200 text-red-800 p-4 rounded-xl text-center">
{errorMsg || 'Customer records missing.'} <div className="mt-4"> <Link to="/customers" className="text-indigo-600 hover:underline">Back to customers ledger</Link> </div> </div>
);
}
...
return ( <div className="space-y-6 max-w-6xl mx-auto font-sans"> <div className="flex items-center space-x-3"> <Link to="/customers" className="p-2 hover:bg-gray-200 rounded-lg transition-colors text-gray-600"> <ArrowLeft className="w-5 h-5" /> </Link> <div> <h2 className="text-2xl font-black text-gray-900">{customer.fullName} Profile & Credit Sheet</h2> <p className="text-sm text-gray-600">Review repeat purchase credibility, cash deals, and active kiston records.</p> </div> </div>

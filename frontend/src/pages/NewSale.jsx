...
const NewSale = () => {
    ...
    const loadWizardOptions = async () => {
      try {
        ...
        if (prodRes.data && prodRes.data.success) {
          const stockAvailable = prodRes.data.data.filter(
            (p) => p.quantity > 0
          );
        ...
        setErrorMsg(
          'Failed to fetch product or customer list from servers.'
        );
      } finally {
  ...
  const activeProductDoc = products.find(
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedCustomer) {
      setErrorMsg(
        'Please select a customer for this checkout invoice.'
      );
    ...
    if (!selectedProduct) {
      setErrorMsg(
        'Please choose an item to finalize checkout.'
      );
      setIsSubmitting(false);
      return;
    }
    ...
    if (quantity > activeProductDoc?.quantity) {
      setErrorMsg(
        `Insufficient stock. Only ${activeProductDoc.quantity} items are available.`
      );
      setIsSubmitting(false);
      return;
    }
    ...
    if (discount > subtotal) {
      setErrorMsg(
        'Discount amount cannot be larger than subtotal.'
      );
      setIsSubmitting(false);
      return;
    }
    ...
    if (
      ...
      setErrorMsg(
        'Down payment cannot exceed final purchase total.'
      );
      setIsSubmitting(false);
      return;
    }
    ...
    try {
      ...
      setErrorMsg(
        err.response?.data?.message ||
          'Transaction could not be compiled on database.'
      );
    } finally {
      setIsSubmitting(false);
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 space-y-3">
        ...
        <span className="text-gray-500 text-sm font-semibold">
          Opening dynamic Checkout terminal...
        </span>
      </div>
    );
  }
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900">
            Checkout Terminal (New Sale)
          </h2>

          <p className="text-sm text-gray-600">
            Finalize cash sales or configure installment structures with custom Bill Number.
          </p>
        </div>
      </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm space-y-4">
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
              ...
              <p className="text-[10px] text-gray-400 mt-1 font-medium">
                * Note: You can enter your paper receipt book number here, or leave empty to let system auto-generate unique ID.
              </p>
            </div>
              ...
              >
                ...
                {customers.map((c) => (
                  <option key={c._id} value={c._id}>
                    ({c.customerId}) {c.fullName} - Mob: {c.mobileNumber}
                  </option>
                ))}
              </select>
            </div>
              ...
              >
                ...
                {products.map((p) => (
                  <option key={p._id} value={p._id}>
                    {p.name} ({p.brand} {p.model}) - Stock Qty: {p.quantity}
                  </option>
                ))}
              </select>
            </div>
            ...
            {selectedProduct && (
              <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 border border-gray-200 rounded-xl">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500">
                    Quantity (Max: {activeProductDoc?.quantity})
                  </label>
            ...
            {paymentType === 'Installment' && (
              <div className="bg-purple-50/50 border border-purple-200 p-4 rounded-xl space-y-4">
                <h4 className="text-sm font-extrabold text-purple-800">
                  Installment Parameters Configurations
                </h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      <option value="3">
                        3 Months Plan (15% Markup)
                      </option>

                      <option value="6">
                        6 Months Plan (25% Markup)
                      </option>

                      <option value="12">
                        12 Months Plan (50% Markup)
                      </option>
                    </select>
                  </div>
                </div>

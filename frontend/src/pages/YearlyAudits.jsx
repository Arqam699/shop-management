...
import { Calendar, Plus, Eye, Trash2, ShieldAlert, X, AlertCircle, Lock } from 'lucide-react';
...
const YearlyAudits = () => {
  ...
  const handleCreateYear = async (e) => {
    ...
    try {
      ...
      setModalError(error.response?.data?.message || 'Failed to create year slot.');
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 font-sans">Lifetime Yearly Audits</h2>
          <p className="text-sm text-gray-600">Digitize your dukan past years manual registers (buying costs, cash/installment sales & profits) cleanly.</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <div className="p-10 text-center col-span-full">Loading auditing registers...</div>
        ) : audits.length === 0 ? (
          <div className="bg-white border rounded-2xl p-10 text-center col-span-full space-y-2 text-gray-500">
            ...
            <p className="font-bold">No historical registers added yet</p>
            <p className="text-xs">Click "Add Year Audit Sheet" to start digitizing registers from 2022 onwards.</p>
          </div>
        ) : (
          audits.map(audit => (
            <div key={audit._id} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between space-y-4">
              <div className="flex items-center justify-between border-b pb-3">
                <div className="flex items-center space-x-2">
                  <Calendar className="w-6 h-6 text-indigo-600" />
                  <span className="text-xl font-black text-slate-800">Year {audit.year}</span>
                </div>
              ...
              <div className="space-y-2 text-xs font-bold text-gray-500">
                <div className="flex justify-between">
                  ...
                  <span className="text-slate-900">{settings.currency} {(audit.totalInventoryCost || 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span>Total Collected Revenue:</span>
                  <span className="text-green-700">{settings.currency} {(audit.totalSalesRevenue || 0).toLocaleString()}</span>
                </div>
                <div className="flex justify-between border-t border-dashed pt-2 text-sm text-slate-900">
                  <span>Net Audited Profit:</span>
                  <span className="text-indigo-600 font-extrabold">{settings.currency} {(audit.totalYearlyProfit || 0).toLocaleString()}</span>
                </div>
              </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <form onSubmit={handleCreateYear} className="bg-white border rounded-2xl w-full max-w-sm shadow-xl overflow-hidden">
            <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
              <span className="font-extrabold text-sm uppercase text-slate-800 tracking-wider">Generate Year Slot</span>
              <button
                type="button"
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold uppercase text-gray-500 font-sans">Enter Year (e.g. 2022, 2023)</label>
                <input
                  type="number"
                  value={newYear}
            ...
            <div className="p-4 bg-gray-50 border-t flex justify-end">
              ...
              >
                {isSubmitting ? 'Generating...' : 'Create slot'}
              </button>
            </div>
          </form>
        </div>

...
// PERMANENT SESSION PRESERVATION SEEDER (Fixes auto-logout on server restart!)
const seedAdminAccount = async () => {
  try {
    ...
    if (!admin) {
      // Create new only if not found (Never wipes out existing records!)
      admin = new Admin({
        email: adminEmail,
      ...
      // If admin exists, verify password without changing admin ID
      ...
      if (!isMatch) {
        ...
        console.log(`[SESSION SAFE] Admin ID preserved. No logouts on restart for: ${adminEmail}`);
      }
    }
  } catch (err) {
